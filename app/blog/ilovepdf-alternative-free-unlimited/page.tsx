...
export const metadata: Metadata = {
  ...
  description: "iLovePDF limits free users to 20MB and adds watermarks. Here is a genuinely free alternative with no file size limits, no watermarks, and no files uploaded to any server.",
...
export default function IlovepdfAlternativePage() {
  ...
  const faqSchema = {
    ...
    mainEntity: [
      ...
      { "@type": "Question", name: "Can I split a password-protected PDF?", acceptedAnswer: { "@type": "Answer", text: "No. You need to remove the password first using the PDF Password Remover. Once the file is unlocked, you can split it. The splitter cannot read encrypted PDFs because the encryption scrambles the page structure. This is a security feature. If the tool could split encrypted PDFs without the password, that would mean the password was useless." } },
    ...
    <>
      ...
      <ArticleWrapper
        ...
        relatedLinks={[
          ...
          { href: "/blog/compress-pdf-without-losing-quality", label: "Compress PDF Without Quality Loss" },
        ]}
      >
        ...
        <p><strong>1. Go to trulyfreetools.com and click PDF Splitter.</strong> The tool loads immediately. No waiting. No "sign up to continue." The interface is obvious: a big upload box, a place to enter page ranges, and a split button. That is it.</p>
        ...
        <p><strong>4. Set your ranges carefully.</strong> If you type "1-5, 6-10, 11-47", the tool will create three PDFs. Do not put spaces after commas unless you want to — it accepts both. The parser is forgiving. But if you type "5-1", it will warn you that the start page must be less than the end page.</p>
        <p><strong>5. Click "Split PDF".</strong> The WebAssembly engine goes to work. It copies the selected pages from the original PDF into new PDF documents. For a 100-page PDF, splitting into 10 ranges takes about 3 seconds. For a 500-page PDF, maybe 10 seconds. The original file remains untouched.</p>
        ...
        <p>Here is the business model, explained plainly. Google AdSense places ads on the homepage and on these blog articles. When you visit, you might see an ad for a VPN or a cloud storage service. I get a few cents. That money pays for the domain (trulyfreetools.com) and the hosting for the static files. I do not run ads on the tool pages themselves because that would be obnoxious. No one wants an ad popping up while they are trying to split a contract.</p>
        ...
        <p>No. You need to remove the password first using the PDF Password Remover. Once the file is unlocked, you can split it. The splitter cannot read encrypted PDFs because the encryption scrambles the page structure. This is a security feature. If the tool could split encrypted PDFs without the password, that would mean the password was useless.</p>
        ...
        <p>Not yet. Currently, the splitter works by page ranges only. If you need to split by file size (e.g., split into 10MB chunks), you would need to experiment with page ranges. I am considering adding this feature if enough people request it. Email me at info@klickifyagency.com if you need it.</p>
